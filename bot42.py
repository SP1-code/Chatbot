...
import csv
import requests  # Om het Excel-bestand te downloaden van GitHub
from io import BytesIO  # Import BytesIO for in-memory file handling

# Laad Excel-bestand vanaf een URL (bijvoorbeeld GitHub) met BytesIO
def load_excel_data_from_url(file_url):
    try:
        response = requests.get(file_url)
        response.raise_for_status()
        file_like = BytesIO(response.content)  # Maak een in-memory bestand

        # Gebruik de juiste engine voor .xlsx-bestanden
...
def main():
    ...
    global data
    file_url = "https://github.com/SP1-code/Chatbot/raw/main/modules_met_uitleg.xlsx"  # Directe downloadlink
    data = load_excel_data_from_url(file_url)
